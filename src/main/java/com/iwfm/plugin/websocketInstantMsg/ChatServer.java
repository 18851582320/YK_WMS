package com.iwfm.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("ADDHBMES")){
			this.userjoin(message.replaceFirst("ADDHBMES", ""),conn);
		}if(null != message && message.startsWith("LeaveHBMES")){
			this.userLeave(conn);
		}if(null != message && message.contains("HBMES886")){
			String toUser = message.substring(message.indexOf("HBMES886")+10, message.indexOf("HBMES888"));
			message = message.substring(0, message.indexOf("HBMES886")) +"[私信]  "+ message.substring(message.indexOf("HBMES888")+12, message.length());
